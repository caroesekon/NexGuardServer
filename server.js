...
const start = async () => {
  ...
  const APP_NAME = process.env.APP_NAME || 'NexGuard';
  ...
  await initDB();
  console.log(`  ${c.white}MongoDB${c.reset}      ${dbStatus ? connected : failed}`);
  ...
  initSocketIO();
  console.log(`  ${c.white}Socket.IO${c.reset}    ${socketStatus ? connected : failed}`);

  initCrons();
  console.log(`  ${c.white}Schedulers${c.reset}   ${cronStatus ? connected : failed}`);

  await checkBridge();
  if (bridgeStatus === true) {
    console.log(`  ${c.white}HDM Bridge${c.reset}   ${connected}`);
  } else if (bridgeStatus === false) {
    console.log(`  ${c.white}HDM Bridge${c.reset}   ${failed}`);
  } else {
    console.log(`  ${c.white}HDM Bridge${c.reset}   ${notConfigured}`);
  }

  await checkAI();
  if (aiStatus === true) {
    console.log(`  ${c.white}HDM AI${c.reset}       ${connected}`);
  } else if (aiStatus === false) {
    console.log(`  ${c.white}HDM AI${c.reset}       ${failed}`);
  } else {
    console.log(`  ${c.white}HDM AI${c.reset}       ${notConfigured}`);
  }
  ...
  server.listen(PORT, () => {
    ...
    console.log(``);
    console.log(`  ${c.white}Server:${c.reset}    ${c.cyan}http://localhost:${PORT}${c.reset}`);
